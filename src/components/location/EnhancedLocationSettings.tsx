...
import { MapPin, Navigation, Eye, EyeOff, Loader2, RefreshCw } from 'lucide-react';
...
export const EnhancedLocationSettings = ({ open, onClose }: EnhancedLocationSettingsProps) => {
  const { user } = useAuth();
  ...
  const loadLocationSettings = async () => {
    ...
    try {
      const { data: profile } = await supabase
        .from('profiles')
        .select('location, preferences, latitude, longitude')
        .eq('user_id', user.id)
        .single();

      if (profile) {
        setCurrentLocation(profile.location || '');
        const prefs = typeof profile.preferences === 'object' ? profile.preferences as any : {};
  ...
  const handleGetCurrentLocation = async () => {
    ...
    try {
      ...
      if (newLocation) {
        const readableLocation = await getReadableLocation(newLocation.lat, newLocation.lng);
        setCurrentLocation(readableLocation);
        
        toast({
          title: "Location Found ✅",
          description: `Current location: ${readableLocation}`,
        });
      }
    } catch (error) {
  ...
  const updateCurrentLocation = async () => {
    ...
    setUpdating(true);
    try {
      const readableLocation = await getReadableLocation(location.lat, location.lng);
