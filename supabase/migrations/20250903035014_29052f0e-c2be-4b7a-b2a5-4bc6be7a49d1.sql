  ...
  AND (
    ...
    OR (
      ...
      AND EXISTS (
        ...
        AND public.calculate_distance(viewer.latitude, viewer.longitude, profiles.latitude, profiles.longitude) <= 100
      )
    )
  )
);
      ...
      AND (
        ...
        OR (
          ...
          AND EXISTS (
            ...
            AND viewer.latitude IS NOT NULL 
            AND viewer.longitude IS NOT NULL
            AND public.calculate_distance(viewer.latitude, viewer.longitude, p.latitude, p.longitude) <= 100
          )
        )
      )
    )
  ...
  AND (
    ...
    OR EXISTS (
      ...
      AND EXISTS (
        ...
        AND viewer.latitude IS NOT NULL 
        AND viewer.longitude IS NOT NULL
        AND public.calculate_distance(viewer.latitude, viewer.longitude, p.latitude, p.longitude) <= 50
      )
    )
  )
);

-- 2. SECURE EVENT ACCESS - require authentication for all event access
...
USING (
  ...
  AND (
    ...
    auth.uid() = creator_id
    -- Or public events (still requires authentication)
    OR is_public = true
  )
);

-- 3. SECURE EVENT ATTENDEES - require authentication
...
-- 4. SECURE STORY MEDIA - require authentication
